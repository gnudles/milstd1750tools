...
co_cpu (int argc, char *argv[])
{
  ...
  if (strmatch (argv[1], "list"))
  ...
  else if (strmatch (argv[1], "create"))
    {
      ...
      int mem_pages = N_PAGES;
      if (argc >= 4)
        {
          mem_pages = atoi(argv[3]);
          if (mem_pages <= 0 || mem_pages > N_PAGES)
            return error ("invalid memory pages value");
        }
      new_cpu->state.num_phys_mem_pages = mem_pages;

      init_simulator (new_cpu, 0);

      lprintf ("Created CPU %d ('%s') with %d memory pages\n", ncpus, new_cpu->name, mem_pages);
      ncpus++;
    }
